...
## 2026-05-01 — Review-gate "stuck on approvals" diagnosis pattern

When a Squad PR shows `squad/review-gate` PENDING with CI green and no reviews/requests visible in `gh pr view`, the cause is almost always: a recent `synchronize` push cleared all `review:{role}:approved` labels (see workflow log line "New commits detected — clearing stale approval labels"), and no new review dispatch was issued. Diagnosis recipe:

1. `squad_reviews_gate_status` — shows which roles are `required: true` with `approved: false`.
2. Cross-check with `.squad/reviews/config.json` `gateRule.required` (`always` vs `conditional`).
3. For `conditional` roles (currently only `docs`), check for bypass labels (`docs:not-needed`, `squad:chore-auto`).
4. Fix is operational, not code: `squad_reviews_dispatch_review` per missing role. Do NOT touch the workflow.

Note: even when the PR author IS the reviewer role's bot (e.g., Amy authoring a docs PR), docs approval is still required unless a bypass label is applied — the gate doesn't auto-skip self-authored roles.
