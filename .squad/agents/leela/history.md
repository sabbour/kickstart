...
- **2026-04-21T02:59:40Z — Ceremony enforcement gap (process):** Declaring a ceremony in `.squad/ceremonies.md` is not sufficient; the coordinator has to treat `before`-ceremony checks as BLOCKING, not advisory. Observed pattern: code-producing agents (fry/bender/@copilot) getting dispatched onto labeled issues that had no DP comment and no DP approval labels. Fix: added a pre-dispatch checklist in `.github/agents/squad.agent.md` → Ceremonies (issue tied? DP posted? DP has leela+zapp+nibbler approvals?) with a negative-pattern example and an explicit positive end-to-end flow. Lesson: any governance rule that lives only in a referenced file and not in the always-loaded coordinator logic will silently erode.

- **2026-04-21T02:59:40Z — Nibbler asymmetry (process):** Having three review dimensions (Leela, Zapp, Nibbler) but only two structured gates (leela/zapp labels posted under the `lead` bot) created a silent two-tier reviewer system — Nibbler ran ad-hoc and Nibbler findings lacked the merge-block teeth Leela/Zapp findings had. Fix: PR Review Gate is now a full four-way structured gate (Leela + Zapp + Nibbler + Docs), Nibbler posts via `gh pr review` under the `lead` bot identity, and Nibbler's charter has an explicit Review Parity Protocol. Lesson: if a reviewer doesn't get a merge-block label on the same authentication path as other reviewers, their review will be treated as optional no matter what the docs say.

- **2026-04-21T02:59:40Z — Docs-gate rationale (process):** Docs drift is a predictable failure mode and it was not being enforced at any ceremony — DPs didn't have to name the docs they'd change, and the PR Review Gate had no docs reviewer dimension. Fix: `Docs impact:` is now a required field on every DP (missing field → automatic rejection; `N/A` must be explicit with justification), and PR Review Gate has a fourth dimension (Docs reviewer — Scribe as interim until a dedicated docs reviewer is cast) with `docs:approved` / `docs:rejected` / `docs:not-applicable` labels in the merge criteria. Lesson: if the only place docs are mentioned is a "Docs and changeset plan" bullet that isn't independently verified, docs will ship stale.
