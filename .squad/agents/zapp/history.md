...
- **2026-05-04T03:21:06-07:00 — PR #454 (squad/mcp-dev-trigger) security review:** Reviewed `.github/workflows/publish-mcp.yml` change adding `dev` to push branches. Confirmed `NPM_TOKEN` remains secret-scoped via `NODE_AUTH_TOKEN` and is not echoed, trigger remains constrained by `packages/mcp-server/**` path filter, and permissions stay least-privilege (`contents: read`, `id-token: write`). Approved via GraphQL review (**APPROVED**) with **Low residual risk** and applied `security:approved` label.
