...
- (2026-04-15 17:05) App Insights observability lane: stacked on PR #312's API startup fix, added IaC for Log Analytics + Application Insights, injected `APPLICATIONINSIGHTS_CONNECTION_STRING` into SWA app settings, exposed anonymous `/api/client-config`, and bootstrapped browser telemetry from runtime config so frontend AJAX and Functions requests land in the same Application Insights instance.
...
- (2026-04-15T17:05:00Z) For Azure Static Web Apps, browser telemetry config should come from a runtime API endpoint, not Vite env or SWA app settings directly. SWA app settings reach the managed Functions environment, but the static frontend only sees values baked at build time, so a small anonymous config endpoint is the cleanest way to share a public Application Insights connection string with the browser.
