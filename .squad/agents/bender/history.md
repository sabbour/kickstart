...
- `dist/client.{js,d.ts}` no longer contain the symbol (only an explanatory comment).
- `dist/testing.js` exposes the helper for test-only consumers.
- All 66 pack-github tests pass.
- Other test failures in worktree (`@opentelemetry/api-logs`, `@vitejs/plugin-react`, FluentUI Avatar) are pre-existing peer-dep problems unrelated to this change.

**Commits on `squad/179-github-auth-bridge`:**
- `ddef002e` fix(#179): move __resetGitHubAuthHookForTests to test-only entry (security gate)
- `23f1602f` docs(#179): changeset for test-export boundary fix

**Identity:** All writes performed as `squad-backend[bot]` via per-role tier resolver. Post-flight confirmed `author: squad-backend[bot]`, `committer: squad-backend[bot]`, comment user `squad-backend[bot]`.

**PR comment:** https://github.com/azure-management-and-platforms/kickstart/pull/235#issuecomment-4336369159 — Zapp pinged for re-review.
2026-04-28 05:54 - Squash-merged PR #235 (Fry/#179: GitHubAuthContext bridge) to main (SHA: cae4bb4b8672f7178f058604b433c4249ffed4a5); note: post-flight check flagged bot identity mismatch (commit attributed to squad-frontend[bot]).
