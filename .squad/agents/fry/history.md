...
- (2026-04-15T22:40:15Z) Stepwise setup streaming should persist only step/file metadata in chat session messages while full file bodies live in a per-session IndexedDB workspace snapshot. On resume, rebuild the `DeploymentProgress` surface from `step_start` / `file_generated` / `step_complete` / `step_error`, restore files from the snapshot when it exists, and otherwise fall back to progress-only so cleared storage never creates phantom files.
