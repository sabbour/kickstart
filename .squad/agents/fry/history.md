...
### 2025-07-28 — Gallery card rendering fix (timing + CSS)

- **Root Cause 1 — useMemo timing**: `GalleryCard`, `WidgetCard`, and `WidgetPreview` all used `useMemo` to call `processMessages()`. But `useA2UI` sets up the `onSurfaceCreated → setSurfaces` subscription in a `useEffect` (runs AFTER render). The `useMemo` fires DURING render — before the subscription is active. Surfaces were created in the processor but the React state callback (`setSurfaces`) never fired, so cards rendered with zero surfaces forever.
- **Root Cause 2 — unstable dependency**: All three components had `a2ui` (the whole hook return object) as a `useMemo` dependency. `useA2UI()` returns a new object each render. For `generate` scenarios this caused infinite loops (each re-run creates new surface IDs via `uid()`). For keyword scenarios the loop self-corrected but still wasted cycles.
- **Root Cause 3 — invalid CSS**: `galleryCard.backgroundColor` was `rgba(${tokens.colorNeutralBackground1}, 0.8)` — but Fluent v9 tokens resolve to CSS variables (`var(--colorNeutralBackground1)`), not raw color values. `rgba(var(...), 0.8)` is invalid CSS → browser ignores it → cards had no background → transparent against gray backdrop.
- **Fix**: Changed `useMemo` → `useEffect` in all three components (GalleryCard, WidgetCard, WidgetPreview). Destructured `{ surfaces, processMessages }` from `useA2UI()` to use stable refs. `processMessages` is stable (wrapped in `useCallback([processor])` where processor comes from a ref). Effects run after the subscription useEffect, so `onSurfaceCreated` correctly triggers `setSurfaces` → re-render with content.
- **Visual polish**: Fixed card backgroundColor to `tokens.colorNeutralBackground1` (solid white). Added `boxShadow: tokens.shadow4` resting + `shadow16` on hover. Added `overflow: hidden`. Added description subtitle (`cardDescription` style). Gallery background changed from `#f8f8f8` to `#f5f5f5`. Masonry gap widened to 20px, max-width 1600px, 4-column breakpoint at 1440px.
