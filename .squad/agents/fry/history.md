...
## Recent Work
- v2 DPs authored: #477 (pack-core), #478 (Playground), #479 (Runner+SSE), #480 (Skill resolver), #482 (pack-azure), #487 (MCP)
...
| #487 MCP rewrite | Phases A→I: harness MCP shell (`buildMcpServer`), `runMcpTurn` turn-collector (buffers chunks+A2UI resources+interrupt), `start`+`converse` core tools, pack-tool pass-through (mcpExposed filter), UserAction named MCP tools as resume mechanism, A2UI embedded resources with `audience:["user"]`, mcp-server entry-point rewrite (delete all v1 files), client stub (deferred TODO), 5 new test files; 6 open questions for Leela+Zapp (interrupt storage contract, structured interrupt signal, credential flow in MCP context, session-to-connection binding, stateless-tool assumption, mcpExposed default tightening) | https://github.com/sabbour/kickstart/issues/487#issuecomment-4269381251 | Proposed — awaiting Leela + Zapp approval |
| #487 DP Revision (Leela + Zapp) | Combined revision addressing Leela C1 (A2UI-over-MCP VS Code-specific scope; `audience:["user"]` not in MCP spec; degradation contract: plain-text fallback; clientInfo detection), C2 (interrupt storage in-memory only; sticky routing required; process-restart 404 documented), C3 (Option 2: `requiresSession:true` tools excluded from MCP manifest entirely); Zapp Crit1 (UserActions NOT MCP tools; Phase E `registerUserActions` removed; structured interrupt block returned instead; MCP client handles consent out-of-band), Crit2 (auth tuple `(sessionId, connectionId, principalId, runId/actionId)`; connectionId server-assigned at initialize; fail-closed before state read/mutate), B1 (`mcpExposed` default `false`; explicit opt-in; FS tools + credential tools + `requiresSession:true` permanently excluded), B2 (zod schema validation same as web path; A2UI payload schema-validated; buffered output through guardrail chain; block → structured error no content), B3 (single-use CAS clear; action-bound 409; TTL 5min 410; preemptive 409; replay guard), B4 (in-memory only; restart → 404; MCP client handles as session-expired; per-session mutex) | https://github.com/sabbour/kickstart/issues/487#issuecomment-4269446489 | Awaiting Leela + Zapp re-check |
