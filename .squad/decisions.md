### 2026-04-16T17:17: User directive
...
# Decision: Engineering Docs Accuracy Rewrite
...
The existing engineering docs (`docs/architecture.md`, `docs/prompt-architecture.md`, `docs-site/docs/architecture/overview.md`) contained several stale or incorrect claims:

1. **Models were wrong** — docs cited GPT-4o/Codex. Current models are GPT-5.4 / GPT-5.4-mini (via `AZURE_OPENAI_CODEX_DEPLOYMENT` and `AZURE_OPENAI_CHAT_DEPLOYMENT`).
2. **Skill injection was underdocumented** — `docs/prompt-architecture.md` described a "Layer 1: Azure Skills (future) — NOT YET IMPLEMENTED" that was actually implemented as two separate mechanisms.
3. **FSM exit conditions were misrepresented** — implied to be code-enforced; they are narrative strings only.
4. **Component count was stale** — "16 custom components" when 28+ are now present.
5. **No docs for `resolveConversationSkills.ts`** (Mechanism B), despite it being live.

## Decision

Rewrote the following files with accurate content:
- `docs/architecture.md` — full engineering overview
- `docs/prompt-architecture.md` — complete LLM pipeline, both mechanisms
- `docs/fsm.md` — new file: FSM enforcement vs LLM-driven behavior
- `docs-site/docs/architecture/overview.md` — Docusaurus version of architecture
- `docs-site/docs/architecture/prompt-pipeline.md` — new: full pipeline with code snippets
- `docs-site/docs/architecture/fsm.md` — new: FSM deep-dive
- `docs-site/docs/architecture/skill-injection.md` — new: side-by-side skill mechanism comparison

## Consequences

- Future contributors can trust the prompt pipeline docs reflect the actual code.
- The "two mechanisms" design is now clearly explained so engineers don't accidentally remove one thinking it's redundant.
- FSM exit condition misconception is explicitly corrected in the docs.
