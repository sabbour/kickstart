 ...
 */
export function processLLMResponse(rawText, phase) {
...
function inferDiscoverComponents(text) {
    const lower = text.toLowerCase();
    // Detect language/runtime/framework question
    if (lower.match(/what\s+(language|runtime|framework|tech)|built\s+(with|in|using)|written\s+in|which\s+(language|framework)/)) {
        return [
            {
                type: "Row",
                gap: "8px",
    ...
    if (lower.match(/existing\s+(code|repo|project)|already\s+have|have\s+a?\s*repo|starting\s+fresh|from\s+scratch|where.+(code|source)/)) {
        return [
            {
                type: "Row",
                gap: "8px",
    ...
    if (lower.match(/what\s+(are\s+you\s+building|does\s+(it|your\s+app)\s+do|kind\s+of\s+app)|tell\s+me\s+about\s+your\s+app|describe/)) {
        return [
            {
                type: "Row",
                gap: "8px",
...
function inferDesignComponents(text) {
    const lower = text.toLowerCase();
