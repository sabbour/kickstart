...
import type { AzureContext } from "@aks-kickstart/harness";
import {
  createResourceGroup,
...
export function decodeRunId(runId: string): DeploymentRunPayload {
  ...
  let payload: DeploymentRunPayload;
  try {
    payload = JSON.parse(Buffer.from(encodedPayload, "base64url").toString("utf8")) as DeploymentRunPayload;
  } catch {
  ...
  if (!payload.expiresAt || Number.isNaN(Date.parse(payload.expiresAt))) {
    throw new AzureApiError(400, "invalid_run_id", "Deployment run ID expiry is invalid.");
  }
...
export async function startAzureDeployment(
  ...
  const runPayload: DeploymentRunPayload = {
    ...
    healthCheckPath,
    startedAt,
    expiresAt: new Date(Date.now() + RUN_ID_TTL_MS).toISOString(),
  };
