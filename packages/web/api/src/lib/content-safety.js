...
export async function checkContentSafety(message) {
    ...
    try {
        const result = await chatCompletion([
            {
                role: "system",
                content: "You are a content safety classifier. Determine if the user message is requesting to build something harmful, illegal, violent, sexually explicit, or otherwise inappropriate for a professional software development context. Reply with ONLY the word 'safe' or 'unsafe'. No explanation.",
            },
            {
                role: "user",
                content: message,
        ...
        if (verdict === "unsafe") {
            return {
                safe: false,
                error: "Your request contains content that falls outside our acceptable use guidelines. Please describe a software application or service idea.",
            };
        }
        return { safe: true };
    }
