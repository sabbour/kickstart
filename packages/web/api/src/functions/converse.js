...
import { Phase, getPhaseDefinition, getPhaseOrder, processResponse, defaultRegistry, buildSystemPrompt, resolveSkills, defaultKitRegistry, } from "@kickstart/core";
    ...
    handler: async (request, context) => {
        ...
        try {
            ...
            if (isTruncated(result)) {
                ...
                const continued = await chatCompletionWithAutoContinue(continueMessages, { responseFormat: { type: "json_object" } }, { continuationPrompt: "Your previous response was cut off mid-JSON. Continue the JSON output exactly where you left off — do not repeat any content." });
...
function handleStreaming(messages, sessionId, engineState, phaseA2ui, context) {
                ...
                for (let round = 0; round < 5 && !toolRoundsComplete; round++) {
                    ...
                    if (probe.finishReason !== "tool_calls" || !probe.toolCalls?.length) {
                        ...
                        if (isTruncated(probe)) {
                            const contMessages = [...workingMessages, { role: "assistant", content: probe.content }];
                            const contResult = await chatCompletionWithAutoContinue(contMessages, { responseFormat: { type: "json_object" } }, { continuationPrompt: "Your previous response was cut off mid-JSON. Continue the JSON output exactly where you left off — do not repeat any content." });
