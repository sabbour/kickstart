...
export function App() {
    ...
    const [mode, setMode] = useState(playgroundEnabled ? 'chat' : 'landing');
    const [sidebarOpen, setSidebarOpen] = useState(false);
    const [isApiAvailable, setIsApiAvailable] = useState(mockEnabled ? true : null);
    ...
    const [streamingSurfaceIds, setStreamingSurfaceIds] = useState([]);
    const streamingSurfaceIdsRef = useRef([]);
    ...
    const handleSendMessage = useCallback(async (text, isAutoContinue = false) => {
    ...
    const handleStartChat = useCallback((prompt) => {
        a2ui.reset();
        fs.clear();
    ...
    if (playgroundEnabled) {
        return (<FluentProvider theme={fluentTheme}>
        <Layout sidebarOpen={false} onToggleSidebar={() => { }} onNewSession={() => { }} showSessionsToggle={false} hasFiles={false}>
          <Playground />
        </Layout>
      </FluentProvider>);
    }
    return (<FluentProvider theme={fluentTheme}>
      <Layout sidebarOpen={sidebarOpen} onToggleSidebar={() => setSidebarOpen(prev => !prev)} onNewSession={handleNewSession} showSessionsToggle={mode === 'chat'} hasFiles={hasFiles} sidebar={mode === 'chat' ? (<SessionsSidebar isOpen={sidebarOpen} onClose={() => setSidebarOpen(false)} sessions={sessions.sessions} activeSessionId={sessions.activeSessionId} onSelectSession={handleResumeSession} onNewSession={handleNewSession}/>) : undefined} fileEditor={mode === 'chat' ? (<FileEditor fs={fs} selectedPath={selectedFile} onSelectFile={setSelectedFile}/>) : undefined}>
        {mode === 'landing' ? (<Landing onStartChat={handleStartChat} recentSessions={sessions.recentSessions} onResumeSession={handleResumeSession} onDeleteSession={sessions.deleteSession} onClearAllSessions={handleClearAllSessions}/>) : (<ChatShell messages={messages} isStreaming={isStreaming} streamingText={currentStreamText} streamingSurfaceIds={streamingSurfaceIds} onSend={handleSendMessage} getSurface={a2ui.getSurface}/>)}
      </Layout>
    </FluentProvider>);
}
