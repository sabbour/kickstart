...
export function SessionsSidebar({ isOpen, onClose, sessions, activeSessionId, onSelectSession, onNewSession, }) {
    return (<aside id="sessions-sidebar" className={`sessions-sidebar${isOpen ? '' : ' hidden'}`} aria-label="Sessions">
      ...
      <div className="sessions-list">
        {sessions.length === 0 && (<div className="session-item" style={{ opacity: 0.5, cursor: 'default' }}>
            No sessions yet
          </div>)}
        {sessions.map(session => (<div key={session.id} className={`session-item${session.id === activeSessionId ? ' active' : ''}`} onClick={() => onSelectSession(session.id)}>
...
//# sourceMappingURL=SessionsSidebar.js.map