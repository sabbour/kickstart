...
import { z } from 'zod';
import { CommonSchemas } from '../../../../web_core/index';
...
export const TextField = createReactComponent(TextFieldApiDef, ({ props, context }) => {
    ...
    return (<div style={{ display: 'flex', flexDirection: 'column', gap: '4px', width: '100%' }}>
      {props.label && (<label htmlFor={id} style={{ fontSize: '14px', fontWeight: 'bold' }}>
          {props.label}
        </label>)}
      {isLong ? (<textarea id={id} style={style} value={props.value || ''} onChange={onChange}/>) : (<input id={id} type={type} style={style} value={props.value || ''} onChange={onChange}/>)}
    </div>);
});
