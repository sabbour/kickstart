...
// ---------------------------------------------------------------------------
// Closed track-ID set (Zapp M1: validate on render side too)
// ---------------------------------------------------------------------------
...
export const TrackPicker = createReactComponent(TrackPickerApi, ({ props, context }) => {
  ...
  const handleSelect = (trackId: string, trackLabel: string) => {
    if (!VALID_TRACK_IDS.has(trackId)) return;
  ...
  return (
    <div data-testid="a2ui-TrackPicker" className={classes.root}>
      ...
      <div className={classes.grid} role="listbox" aria-label={String(props.title)}>
        {tracks.map((track) => (
          <Card
            key={track.id}
