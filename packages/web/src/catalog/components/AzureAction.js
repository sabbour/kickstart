...
import { Body1, Body2, Button, Card, CardHeader, Caption1, Field, Input, MessageBar, MessageBarBody, Spinner, Subtitle2, makeStyles, tokens, } from '@fluentui/react-components';
...
export const AzureAction = createReactComponent(AzureActionApi, ({ props }) => {
    ...
    const executeAction = async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                const resourceId = resultData?.id;
                setState('success');
                setResultMessage(resourceId
    ...
    return (<Card className={classes.root}>
      ...
      {/* Validation error */}
      {pathError && (<MessageBar intent="error" className={classes.validationError}>
          <MessageBarBody>
      ...
      {state !== 'confirming' && (<div className={classes.actions}>
          ...
          {state === 'idle' && (<Button appearance={isDestructive ? 'primary' : 'primary'} style={isDestructive ? { backgroundColor: tokens.colorPaletteRedBackground3 } : undefined} onClick={handleConfirm} disabled={!!pathError} icon={<ShieldCheckmark20Regular />}>
              {confirmLabel}
            </Button>)}
...
//# sourceMappingURL=AzureAction.js.map