...
import { z } from 'zod';
import { DynamicStringSchema, ComponentIdSchema, } from '../../vendor/a2ui/web_core/schema/common-types';
import { Card, Button, Subtitle2, Caption1, makeStyles, tokens, } from '@fluentui/react-components';
import { ChevronLeftRegular, ChevronRightRegular, CheckmarkRegular, } from '@fluentui/react-icons';
    ...
    return (<Card className={classes.root}>
      ...
      <div className={classes.indicators}>
        {props.steps.map((step, i) => {
            ...
            if (i === clampedStep)
                pillClass = classes.pillActive;
            else if (i < clampedStep)
                pillClass = classes.pillCompleted;
            return (<div key={i} className={classes.indicator}>
      ...
      <div className={classes.footer}>
        ...
        <Button appearance="subtle" icon={<ChevronLeftRegular />} disabled={isFirst} onClick={() => setCurrentStep(s => Math.max(0, s - 1))}>
          Previous
        </Button>

        <Button appearance="primary" icon={isLast ? <CheckmarkRegular /> : <ChevronRightRegular />} iconPosition="after" onClick={() => {
            if (!isLast)
...
//# sourceMappingURL=SteppedCarousel.js.map