...
import { Body2, Button, Caption1, Card, CardHeader, Checkbox, Field, Input, Link, MessageBar, MessageBarBody, Spinner, Subtitle2, Textarea, Tooltip, makeStyles, tokens, } from '@fluentui/react-components';
import { CheckmarkCircle20Regular, DismissCircle20Regular, Document20Regular, } from '@fluentui/react-icons';
...
function validateBranchName(name) {
    ...
    if (name.endsWith('.lock') || name.endsWith('.'))
        return 'Branch name cannot end with .lock or .';
    if (name.includes('..') || name.includes(' '))
        return 'Branch name cannot contain spaces or ..';
    if (PROTECTED_BRANCHES.has(name))
        return `Cannot push directly to protected branch "${name}"`;
    return null;
}
...
export const GitHubCommit = createReactComponent(GitHubCommitApi, ({ props }) => {
    ...
    const [selectedFiles, setSelectedFiles] = useState(() => new Set(artifacts.map((a) => a.path)));
    ...
    const selectedArtifacts = useMemo(() => artifacts.filter((a) => selectedFiles.has(a.path)), [artifacts, selectedFiles]);
    ...
    const handleCreatePR = async () => {
        ...
        try {
            ...
            const pr = await connector.createPullRequest(owner, repo, prTitle, branchName, defaultBranch, prBodyFull);
            setState('success');
    ...
    if (state === 'success' || state === 'error') {
        ...
        <CardHeader header={<Subtitle2>Create Pull Request</Subtitle2>} description={<Caption1>
        ...
        {prUrl && (<div style={{ marginTop: tokens.spacingVerticalS }}>
    ...
    if (state === 'executing') {
        return (<Card className={classes.root}>
        <CardHeader header={<Subtitle2>Creating Pull Request\u2026</Subtitle2>} description={<Caption1>Pushing {selectedArtifacts.length} files to {branchName}</Caption1>}/>
    ...
    if (state === 'configuring') {
        return (<Card className={classes.root}>
        <CardHeader header={<Subtitle2>Configure Pull Request</Subtitle2>} description={<Caption1>
              {selectedArtifacts.length} file{selectedArtifacts.length !== 1 ? 's' : ''} selected
            </Caption1>}/>

        {error && (<MessageBar intent="error" style={{ marginTop: tokens.spacingVerticalS }}>
            <MessageBarBody>{error}</MessageBarBody>
          </MessageBar>)}

        <div className={classes.formFields}>
          <Field label="Target repository" validationState={repoFullName ? 'none' : 'warning'} validationMessage={repoFullName ? undefined : 'No repository selected'}>
            <Input value={repoFullName} disabled style={{ fontFamily: tokens.fontFamilyMonospace }}/>
          </Field>

          <Field label="Branch name" validationState={branchError ? 'error' : 'success'} validationMessage={branchError ?? 'Valid branch name'}>
            <Input value={branchName} onChange={(_, data) => setBranchName(data.value)} style={{ fontFamily: tokens.fontFamilyMonospace }}/>
          </Field>

          <Field label="PR title">
            <Input value={prTitle} onChange={(_, data) => setPrTitle(data.value)}/>
          </Field>

          <Field label="PR description">
            <Textarea value={prBody} onChange={(_, data) => setPrBody(data.value)} rows={4} resize="vertical"/>
          </Field>
        </div>
        ...
        <div className={classes.actions}>
          ...
          <Button appearance="primary" onClick={handleCreatePR} disabled={!!branchError || !prTitle || selectedArtifacts.length === 0}>
            Create pull request
          </Button>
        </div>

        {!connector && (<Caption1 style={{ color: tokens.colorNeutralForeground3, marginTop: tokens.spacingVerticalS }}>
    ...
    return (<Card className={classes.root}>
      ...
      {artifacts.length === 0 ? (<MessageBar intent="warning" style={{ marginTop: tokens.spacingVerticalS }}>
          ...
          <div className={classes.artifactList}>
            {artifacts.map((artifact) => (<div key={artifact.path}>
                <div className={classes.artifactItem}>
                  <Checkbox checked={selectedFiles.has(artifact.path)} onChange={() => toggleFile(artifact.path)} aria-label={`Select ${artifact.path}`}/>
                  ...
                  <Tooltip content={artifact.path} relationship="description">
                    <span className={classes.artifactPath} style={{ cursor: 'pointer' }} onClick={() => setExpandedFile(expandedFile === artifact.path ? null : artifact.path)}>
                      {artifact.path}
                    </span>
                  </Tooltip>
                ...
                {expandedFile === artifact.path && (<pre className={classes.diffPreview}>
      ...
      <div className={classes.actions}>
        <Button appearance="primary" onClick={handleContinue} disabled={selectedFiles.size === 0}>
