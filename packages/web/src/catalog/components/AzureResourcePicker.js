...
import { Body2, Caption1, Card, CardHeader, Field, Input, Select, Spinner, Badge, MessageBar, MessageBarBody, makeStyles, shorthands, tokens, } from '@fluentui/react-components';
...
export const AzureResourcePicker = createReactComponent(AzureResourcePickerApi, ({ props }) => {
    ...
    const [selectedSubId, setSelectedSubId] = useState(presetSubId ?? '');
    const [resourceGroups, setResourceGroups] = useState([]);
    const [selectedRg, setSelectedRg] = useState(presetRg ?? '');
    ...
    return (<div className={classes.root}>
      ...
      {(showSubDropdown || showRgDropdown) && (<div className={classes.cascadeRow}>
          {showSubDropdown && (<Field label="Subscription" className={classes.cascadeField}>
              {loadingSubs ? (<Spinner size="tiny" label="Loading…"/>) : (<Select value={selectedSubId} onChange={(_, data) => setSelectedSubId(data.value)}>
                  <option value="">Select a subscription…</option>
                  {subscriptions.map((sub) => (<option key={sub.subscriptionId} value={sub.subscriptionId}>
                      {sub.displayName}
                    </option>))}
                </Select>)}
            </Field>)}

          {showRgDropdown && (<Field label="Resource Group" className={classes.cascadeField}>
              {loadingRgs ? (<Spinner size="tiny" label="Loading…"/>) : (<Select value={selectedRg} onChange={(_, data) => setSelectedRg(data.value)}>
                  <option value="">All resource groups</option>
                  {resourceGroups.map((rg) => (<option key={rg.name} value={rg.name}>
                      {rg.name} ({rg.location})
                    </option>))}
                </Select>)}
            </Field>)}
      ...
      {selectedSubId && (<div className={classes.searchRow}>
          <Input contentBefore={<Search20Regular />} placeholder="Search resources…" value={searchQuery} onChange={(_, data) => setSearchQuery(data.value)} style={{ width: '100%' }}/>
        </div>)}

      {error && (<MessageBar intent="error" style={{ marginBottom: tokens.spacingVerticalS }}>
          <MessageBarBody>{error}</MessageBarBody>
        </MessageBar>)}

      {loadingResources ? (<div className={classes.spinnerRow}>
          <Spinner size="small" label="Loading resources…"/>
        </div>) : !selectedSubId ? (<Caption1 className={classes.emptyText}>Select a subscription to browse resources.</Caption1>) : filteredResources.length === 0 ? (<Caption1 className={classes.emptyText}>No resources found.</Caption1>) : (<div className={classes.resourceList}>
          {filteredResources.map((resource) => (<Card key={resource.id} className={selected === resource.id ? classes.resourceCardSelected : classes.resourceCard} onClick={() => handleSelect(resource)} role="option" aria-selected={selected === resource.id}>
              <CardHeader header={<Body2 style={{ fontWeight: 600 }}>{resource.name}</Body2>} description={<Caption1>
...
//# sourceMappingURL=AzureResourcePicker.js.map