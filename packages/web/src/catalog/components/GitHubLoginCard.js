...
export const GitHubLoginCard = createReactComponent(GitHubLoginCardApi, ({ props }) => {
    ...
    if (phase === 'authenticated') {
        ...
        <CardHeader header={<Body1Strong>GitHub</Body1Strong>} description={<Caption1>
              <span className={classes.statusDot}/>
              Connected
            </Caption1>}/>
        <div className={classes.signedIn}>
          <Avatar image={avatarSrc ? { src: avatarSrc } : undefined} name={displayName} size={36}/>
        ...
        {authTime && (<div className={classes.tokenInfo}>
    ...
    if (phase === 'awaiting-code' || phase === 'polling') {
        return (<Card className={classes.root}>
        <CardHeader header={<Body1Strong>GitHub</Body1Strong>} description={<Caption1>Authenticating via device code flow\u2026</Caption1>}/>
        <div className={classes.deviceCodeSection}>
          {deviceCode ? (<>
              <Caption1>
                Go to{' '}
                <a href={deviceCode.verificationUri} target="_blank" rel="noopener noreferrer" style={{ color: tokens.colorBrandForeground1 }}>
        ...
        {phase === 'polling' && (<div className={classes.actions} style={{ justifyContent: 'center' }}>
            <Spinner size="tiny"/>
    ...
    // Idle / sign-in state
    return (<Card className={classes.root}>
      <CardHeader header={<Body1Strong>GitHub</Body1Strong>} description={<Caption1>Sign in to connect your repositories</Caption1>}/>
      {error && (<MessageBar intent="error">
          <MessageBarBody>{error}</MessageBarBody>
        </MessageBar>)}
      <div className={classes.actions}>
        <Button appearance="primary" onClick={handleSignIn} disabled={phase !== 'idle'} icon={phase !== 'idle' ? <Spinner size="tiny"/> : undefined}>
          Sign in with GitHub
        </Button>
      </div>
...
//# sourceMappingURL=GitHubLoginCard.js.map