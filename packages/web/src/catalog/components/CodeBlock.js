...
import { Card, Button, Body1Strong, Caption1, makeStyles, tokens, } from '@fluentui/react-components';
...
export const CodeBlock = createReactComponent(CodeBlockApi, ({ props }) => {
    ...
    return (<Card className={classes.root}>
      {(props.filename || props.language) && (<div className={classes.header}>
          <div className={classes.fileInfo}>
            {props.filename && <Body1Strong>{props.filename}</Body1Strong>}
            {props.language && (<Caption1>{props.language}</Caption1>)}
          </div>
          <Button appearance="subtle" icon={copied ? <CheckmarkRegular /> : <CopyRegular />} onClick={handleCopy} size="small">
            {copied ? 'Copied' : 'Copy'}
          </Button>
        </div>)}
