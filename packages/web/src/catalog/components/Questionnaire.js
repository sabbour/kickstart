...
import { Button, Checkbox, Input, Label, RadioGroup as FluentRadioGroup, Radio, Card, Subtitle2, makeStyles, tokens, } from '@fluentui/react-components';
...
export const Questionnaire = createReactComponent(QuestionnaireApi, ({ props }) => {
    ...
    return (<Card className={classes.root}>
      ...
      {(props.questions || []).map((q) => {
            ...
            return (<div key={qId} className={classes.question}>
            <Label className={classes.label}>
              {qLabel}
              {q.required && <span className={classes.required}>*</span>}
            </Label>

            {qType === 'text' && (<Input style={{ width: '100%' }} value={answers[qId] || ''} onChange={(_e, data) => updateAnswer(qId, data.value)}/>)}

            {qType === 'choice' && q.choices && (<FluentRadioGroup value={answers[qId] || ''} onChange={(_e, data) => updateAnswer(qId, data.value)}>
                {q.choices.map((c) => (<Radio key={c.id} value={c.id} label={c.label}/>))}
              </FluentRadioGroup>)}

            {qType === 'multiChoice' && q.choices && (<div className={classes.checkboxGroup}>
                {q.choices.map((c) => {
                        const selected = (answers[qId] || []).includes(c.id);
                        return (<Checkbox key={c.id} checked={selected} onChange={() => toggleMultiChoice(qId, c.id)} label={c.label}/>);
                    })}
              </div>)}
          </div>);
        })}
