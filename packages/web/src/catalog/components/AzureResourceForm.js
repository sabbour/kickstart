...
import { Body1, Button, Card, CardHeader, Caption1, Field, Input, Select, Spinner, Subtitle2, Switch, MessageBar, MessageBarBody, makeStyles, tokens, } from '@fluentui/react-components';
...
export function sanitizeArmPathSegment(segment) {
...
function validateResourceGroup(value) {
    if (!RESOURCE_GROUP_RE.test(value))
        return 'Resource group: 1-90 chars, alphanumeric/hyphens/underscores/periods/parentheses';
    if (!RESOURCE_GROUP_NO_TRAILING_DOT_RE.test(value))
        return 'Resource group name cannot end with a period';
    return null;
}
function validateResourceName(value) {
    if (!RESOURCE_NAME_RE.test(value))
        return 'Resource name: 1-90 chars, starts with alphanumeric, allows alphanumeric/periods/hyphens/underscores';
    return null;
}
...
function getDefaultFields(resourceType) {
    ...
    if (typeLower.includes('managedclusters')) {
        fields.push({ name: 'kubernetesVersion', label: 'Kubernetes version', type: 'enum', required: false, options: ['1.31', '1.30', '1.29', '1.28'] }, { name: 'dnsPrefix', label: 'DNS prefix', type: 'string', required: false }, { name: 'enableRBAC', label: 'Enable RBAC', type: 'boolean', required: false });
    }
    else if (typeLower.includes('registries')) {
        fields.push({ name: 'adminUserEnabled', label: 'Admin user', type: 'boolean', required: false });
    }
    else if (typeLower.includes('storageaccounts')) {
        fields.push({ name: 'accessTier', label: 'Access tier', type: 'enum', required: false, options: ['Hot', 'Cool', 'Archive'] }, { name: 'httpsOnly', label: 'HTTPS only', type: 'boolean', required: false });
    }
    return fields;
}
...
export const AzureResourceForm = createReactComponent(AzureResourceFormApi, ({ props, context }) => {
    ...
    const renderField = (field) => {
        ...
        switch (field.type) {
            case 'boolean':
                return (<Field key={field.name} label={field.label}>
            <Switch checked={!!value} onChange={(_, data) => setFieldValue(field.name, data.checked)} disabled={submitting}/>
          </Field>);
            case 'enum':
                return (<Field key={field.name} label={field.label} required={field.required}>
            <Select value={String(value ?? '')} onChange={(_, data) => setFieldValue(field.name, data.value)} disabled={submitting}>
            ...
            default:
                return (<Field key={field.name} label={field.label} required={field.required}>
            <Input type={field.type === 'integer' ? 'number' : 'text'} placeholder={field.description ?? `Enter ${field.label.toLowerCase()}`} value={String(value ?? '')} onChange={(_, data) => setFieldValue(field.name, data.value)} disabled={submitting}/>
          </Field>);
        }
    };
    return (<Card className={classes.root}>
      ...
      <form onSubmit={handleSubmit} className={classes.form}>
        ...
        <Field label="Location">
          {loadingLocations ? (<Spinner size="tiny" label="Loading locations…"/>) : (<Select value={location} onChange={(_, data) => setLocation(data.value)} disabled={submitting}>
        ...
        <Field label="SKU">
          <Select value={sku} onChange={(_, data) => setSku(data.value)} disabled={submitting}>
            {AZURE_SKUS.map((s) => (<option key={s} value={s}>
                {s}
              </option>))}
          </Select>
        </Field>
        ...
        {subscriptionIdError && (<MessageBar intent="error">
            <MessageBarBody>{subscriptionIdError}</MessageBarBody>
          </MessageBar>)}
        {resourceGroupError && (<MessageBar intent="error">
            <MessageBarBody>{resourceGroupError}</MessageBarBody>
          </MessageBar>)}

        {success && (<Caption1 className={classes.successMsg}>
        ...
        <div className={classes.actions}>
          <Button appearance="subtle" type="button" onClick={handleReset} disabled={submitting}>
            Reset
          </Button>
          <Button appearance="primary" type="submit" disabled={submitting || !resourceName.trim() || !!subscriptionIdError || !!resourceGroupError} icon={submitting ? <Spinner size="tiny"/> : undefined}>
...
//# sourceMappingURL=AzureResourceForm.js.map