...
test.describe('Phase A triage track picker', () => {
  test('keeps the shared triage surface stable across pick_track and select_inference', async ({ page }) => {
    ...
    // TrackPicker renders with title and 4 equal-weight tiles
    ...
    await expect(page.getByRole('option', { name: /static site/i })).toBeVisible();
    await expect(page.getByRole('option', { name: /containerized web app/i })).toBeVisible();
    await expect(page.getByRole('option', { name: /agentic ai app/i })).toBeVisible();
    await expect(page.getByRole('option', { name: /existing repo uplift/i })).toBeVisible();
    ...
    await page.getByRole('option', { name: /agentic ai app/i }).click();
