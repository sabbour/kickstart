...
export const test = base.extend({
    mockAuth: [async ({ page }, use) => {
            ...
            await page.route('**/msal-browser*', route => route.fulfill({
                status: 200,
        ...
        `,
            ...
            await page.route('**/api/**', route => route.fulfill({ status: 503, contentType: 'text/plain', body: 'No backend' }));
            // Intercept Fluent UI CDN — not needed for functional tests
            await page.route('**/unpkg.com/@fluentui/**', route => route.fulfill({ status: 200, contentType: 'application/javascript', body: '/* noop */' }));
            await use();
        }, { auto: true }],
});
...
export async function waitForAssistantMessage(page, partialText, timeoutMs = 8000) {
...
export async function enterChatViaTrack(page, track) {
