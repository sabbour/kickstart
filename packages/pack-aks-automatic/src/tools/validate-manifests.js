...
export function staticValidateManifest(yaml) {
    const diagnostics = [];
...
// ── kubectl dry-run ───────────────────────────────────────────────────────────
export async function kubectlDryRun(yaml) {
...
// ── Tool ──────────────────────────────────────────────────────────────────────
export const validateManifestsTool = {
    name: 'aks.validate_manifests',
    tool: tool({
        name: 'aks.validate_manifests',
        description: 'Validates a Kubernetes YAML manifest using kubectl --dry-run=client. ' +
            ...
            if (dryRun.toolMissing) {
                return {
                    ...
                    diagnostics: [
                        {
                            severity: 'error',
                            message: 'kubectl is not available on this server — manifest validation requires kubectl',
                        },
                    ],
