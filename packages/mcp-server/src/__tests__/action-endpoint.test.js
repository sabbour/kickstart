...
// ── Helpers ─────────────────────────────────────────────────────────
function createSession(id, phase = Phase.Discover) {
...
async function sendAction(sessions, payload) {
...
describe("B-24: /api/action endpoint — valid requests", () => {
    ...
    it("returns A2UI component in the response", async () => {
        ...
        const result = await handleAppMessage({ type: "action", sessionId: "sess-a2ui", actionType: "advance", payload: {} }, sessions, "kickstart");
        expect(result.type).toBe("response");
        if (result.type === "response") {
...
describe("B-24: /api/action endpoint — malformed payloads (400)", () => {
    ...
    it("rejects null input", async () => {
        const result = await sendAction(sessions, null);
        expect(result.type).toBe("error");
    });
    it("rejects non-object input", async () => {
        const result = await sendAction(sessions, "not-an-object");
        expect(result.type).toBe("error");
    });
...
//# sourceMappingURL=action-endpoint.test.js.map