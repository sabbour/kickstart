...
// ── Helpers ─────────────────────────────────────────────────────────
function createSession(id, phase = Phase.Discover, overrides = {}) {
...
describe("B-23: A2UI action handler — reply actions", () => {
    ...
    it('"reply" action adds a user message to the conversation', async () => {
        ...
        await handleAction(sessions, "s-reply", "reply", {
        ...
        expect(userMessages[userMessages.length - 1].content).toContain("Yes, I want a Node.js app");
    });
    it('"reply" action does not advance the phase', async () => {
        ...
        await handleAction(sessions, "s-reply-phase", "reply", {
    ...
    it('"reply" action returns an assistant response (re-prompt)', async () => {
        ...
        const result = await handleAction(sessions, "s-reply-resp", "reply", {
    ...
    it('"reply" without a message payload returns a validation error', async () => {
        ...
        const result = await handleAction(sessions, "s-reply-empty", "reply");
        const text = extractText(result);
    ...
    it('"reply" updates the session updatedAt timestamp', async () => {
        ...
        await handleAction(sessions, "s-reply-ts", "reply", {
            message: "Update me",
        });
...
describe("B-23: A2UI action handler — navigate actions", () => {
    ...
    it('"navigate" action transitions to the specified phase', async () => {
        ...
        await handleAction(sessions, "s-nav", "navigate", {
    ...
    it('"navigate" returns an A2UI phase indicator resource', async () => {
        ...
        const result = await handleAction(sessions, "s-nav-a2ui", "navigate", {
            targetPhase: Phase.Design,
        });
    ...
    it('"navigate" to an invalid phase returns an error', async () => {
        ...
        const result = await handleAction(sessions, "s-nav-bad", "navigate", {
    ...
    it('"navigate" without targetPhase returns an error', async () => {
        ...
        const result = await handleAction(sessions, "s-nav-missing", "navigate");
        const text = extractText(result);
    ...
    it('"navigate" can go backward to an earlier phase', async () => {
        ...
        await handleAction(sessions, "s-nav-back", "navigate", {
...
describe("B-23: A2UI action handler — api actions", () => {
    ...
    it('"api" action does not throw or crash', async () => {
        ...
        // Should not throw
        const result = await handleAction(sessions, "s-api", "api", {
    ...
    it('"api" action returns a stub/placeholder response', async () => {
        ...
        const result = await handleAction(sessions, "s-api-stub", "api", {
    ...
    it('"api" action does not change the session phase', async () => {
        ...
        await handleAction(sessions, "s-api-phase", "api", {
            endpoint: "/clusters",
            method: "POST",
        });
...
describe("B-23: A2UI action handler — unknown and invalid actions", () => {
    ...
    it("unknown action type returns an error without crashing", async () => {
        ...
        const result = await handleAction(sessions, "s-unknown", "foobar", { data: "test" });
        expect(result).toBeDefined();
        const text = extractText(result);
    ...
    it("unknown action type does not mutate session state", async () => {
        ...
        await handleAction(sessions, "s-unk-mut", "definitely-not-real", {
            dangerous: "payload",
        });
    ...
    it("empty string action type is handled gracefully", async () => {
        ...
        const result = await handleAction(sessions, "s-empty-type", "");
        expect(result).toBeDefined();
        expect(result.content.length).toBeGreaterThan(0);
    });
});
describe("B-23: A2UI action handler — past-turn action rejection", () => {
    ...
    it("action with expired turnId is rejected", async () => {
        ...
        const result = await handleAction(sessions, "s-past", "select", {
            runtime: "python",
            _turnId: 1,
            _currentTurn: 5,
        });
    ...
    it("action with matching turnId is accepted", async () => {
        ...
        await handleAction(sessions, "s-current-turn", "select", {
            runtime: "node",
            _turnId: 3,
            _currentTurn: 3,
        });
...
//# sourceMappingURL=action-handler.test.js.map