...
describe("handleAppMessage", () => {
  ...
  it("handles kickstart message and returns a response with sessionId", async () => {
    const msg: AppToServerMessage = { type: "kickstart" };
  ...
  it("handles converse message for an existing session", async () => {
    ...
    const sessionId = (kickResult as any).sessionId;

    // Then converse
    const converseResult = await handleAppMessage(
      { type: "converse", sessionId, message: "I have a Node.js app" },
      sessions,
    );
  ...
  it("returns error for converse with unknown session", async () => {
    const result = await handleAppMessage(
      { type: "converse", sessionId: "nonexistent", message: "hello" },
      sessions,
    );
  ...
  it("handles action message for an existing session", async () => {
    ...
    const result = await handleAppMessage(
      { type: "action", sessionId, actionType: "select", payload: { runtime: "nodejs" } },
      sessions,
    );
    expect(result.type).toBe("response");
  });
  ...
  it("returns A2UI component in kickstart response", async () => {
    ...
    if (result.type === "response") {
      ...
      expect((result.a2ui as any).type).toBe("ConversationPhase");
    }
  });
