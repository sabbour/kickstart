...
export const KICKSTART_CATALOG_ID = "https://kickstart.aks.azure.com/catalog/v1/kickstart-catalog.json";
/**
 ...
 */
export function resolveA2UICapability(clientCatalogs) {
    if (!clientCatalogs || clientCatalogs.length === 0)
        return "none";
    if (clientCatalogs.includes(KICKSTART_CATALOG_ID))
        return "kickstart";
 ...
 */
export function createA2UIDocument(root) {
    return {
        version: "0.9",
        root,
    };
 ...
 */
export function degradeToBasic(root, title) {
    const text = {
        type: "Text",
 ...
 */
export function createA2UIResource(root, uri, capability = "kickstart") {
    if (capability === "none")
        return null;
