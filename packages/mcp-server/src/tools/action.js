 ...
 */
import { transition, getCurrentPhase, getPhaseDefinition, getPhaseOrder, Phase, } from "@kickstart/core";
 ...
 */
export async function handleAction(sessions, sessionId, actionType, payload) {
    ...
    for (const phase of phases) {
        ...
        else if (phases.indexOf(phase) < phases.indexOf(session.currentPhase)) {
    ...
    let engineState = {
        ...
        phaseStatus,
        phaseData: Object.fromEntries(phases.map((p) => [p, {}])),
        isComplete: false,
    };
    // Process action
    switch (actionType) {
        ...
        case "reply": {
            ...
            session.updatedAt = new Date().toISOString();
            sessions.set(sessionId, session);
            const replyDef = getPhaseDefinition(session.currentPhase);
            return {
        ...
        case "navigate": {
            ...
            const navPhaseStatus = {};
            for (const phase of navPhases) {
                if (phase === session.currentPhase) {
                    navPhaseStatus[phase] = "active";
                }
                else if (navPhases.indexOf(phase) < navPhases.indexOf(session.currentPhase)) {
            ...
            const navPhaseItems = navPhases.map((phase) => ({
                id: phase,
                label: getPhaseDefinition(phase).label,
                status: navPhaseStatus[phase],
            }));
            const navPhaseComponent = {
                type: "ConversationPhase",
                id: "phase-indicator",
                phases: navPhaseItems,
                currentPhase: session.currentPhase,
            };
            const navResource = createA2UIResource(navPhaseComponent, `a2ui://kickstart/session/${sessionId}/phase`);
            const navDef = getPhaseDefinition(session.currentPhase);
            const navContent = [
                { type: "text", text: `Navigated to **${navDef.label}** phase: ${navDef.description}` },
            ];
            if (navResource)
                navContent.push(navResource);
    ...
    const phaseItems = getPhaseOrder().map((phase) => ({
        id: phase,
        label: getPhaseDefinition(phase).label,
    ...
    const a2uiResource = createA2UIResource(phaseComponent, `a2ui://kickstart/session/${sessionId}/phase`);
