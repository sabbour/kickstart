 ...
 */
import { generateKubernetesManifests, generateGitHubActionsWorkflow, DEPLOYMENT_SAFEGUARDS, } from "@kickstart/core";
import { createA2UIResource } from "../a2ui.js";
/** Run lightweight static checks against generated K8s manifest content. */
function validateManifests(files, tier) {
    const yamlContent = files
        .filter((f) => f.language === "yaml")
        .map((f) => f.content)
        .join("\n---\n");
    return DEPLOYMENT_SAFEGUARDS.map((sg) => {
        switch (sg.rule) {
...
function buildSafeguardCard(results) {
 ...
 */
export async function handleGenerateManifests(sessions, sessionId, capability = "kickstart") {
    ...
    const app = session.appDefinition;
    const azure = session.azureContext;
    ...
    const codeBlocks = allOutputs.flatMap((output) => output.files.map((file) => ({
        type: "CodeBlock",
    ...
    const manifestResource = createA2UIResource(uiColumn, `a2ui://kickstart/session/${sessionId}/manifests`, capability);
    if (manifestResource)
        content.push(manifestResource);
    // Add safeguard validation card
    const safeguardCard = buildSafeguardCard(safeguardResults);
    const safeguardResource = createA2UIResource(safeguardCard, `a2ui://kickstart/session/${sessionId}/safeguards`, capability);
    if (safeguardResource)
        content.push(safeguardResource);
...
//# sourceMappingURL=generate-manifests.js.map