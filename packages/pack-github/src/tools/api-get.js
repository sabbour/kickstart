...
export function validateGithubPath(rawPath) {
...
const ApiGetInputSchema = z.object({
    path: z
        .string()
        .describe('GitHub API path, e.g. /repos/{owner}/{repo} or /users/{username}. Must start with /.'),
    params: z
...
export const apiGetTool = {
    ...
    tool: tool({
        name: 'github.api_get',
        description: 'Read-only GitHub REST API GET. Returns parsed JSON. ' +
        ...
        execute: async (input, runCtx) => {
            ...
            const token = session?.tokens?.['github'];
            if (!token) {
                throw new Error('No GitHub token found in session. Please authenticate first via github:login.');
            }
