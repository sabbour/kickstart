...
export const BASE_COMPONENT_CATALOG: readonly ComponentCatalogEntry[] = [
  ...
  // -- Domain ---------------------------------------------------------------
  {
    type: "CostEstimate",
    category: "domain",
    example: '{"id":"cost1","component":"CostEstimate","title":"Estimated Monthly Cost","resources":[{"name":"AKS Automatic control plane","sku":"Standard","monthlyEstimate":116.80},{"name":"Workload compute","sku":"Standard_D2s_v5","monthlyEstimate":81.53},{"name":"App Routing","monthlyEstimate":0,"pricingModel":"included"},{"name":"Azure OpenAI","sku":"GPT-4.1 mini","monthlyEstimate":0,"pricingModel":"usage","unitPrice":0.00044,"unitOfMeasure":"1K tokens"}],"monthlyEstimate":198.33,"currency":"USD","source":"live","citation":"Prices from Azure Retail Prices API (East US, consumption). Usage-based rows are excluded from the monthly total until usage is known.","loading":{"supported":true,"state":"loading","message":"Fetching live prices from Azure Retail Prices API…"},"cache":{"status":"miss"},"fallback":{"used":false},"pricingRequest":{"region":"eastus","lineItems":[{"id":"aks-control-plane","kind":"aksAutomaticControlPlane"},{"id":"workload-compute","kind":"aksAutomaticWorkloadCompute","sku":"Standard_D2s_v5","quantity":1},{"id":"app-routing","kind":"appRouting"},{"id":"openai","kind":"azureOpenAI","sku":"gpt-4.1-mini"}]}}',
    notes: "Canonical shape: resources[].monthlyEstimate + monthlyEstimate. Use source + citation + loading + cache + fallback to show pricing provenance. Add pricingRequest with allowlisted line-item kinds only when the web client should fetch live Azure Retail Prices API data for the active session.",
  },
  {
    ...
    notes: 'Prefer the `diagram` prop (raw Mermaid) over `nodes/edges` — it supports nested subgraphs, ELK layout, multiline subtitles, and strict `%%icon:name%%` placeholders. Use available shared icon keys like `azure/aks`, `azure/acr`, `azure/postgresql`, `azure/key-vault`, `k8s/ns`, `k8s/deploy`, `k8s/svc`, `k8s/sa`, `k8s/hpa`, `k8s/gateway`, `k8s/httproute`, `k8s/pdb`, `k8s/vpa`, `k8s/cronjob`, `k8s/role`, `k8s/rb`, `k8s/deviceclass`, `k8s/resourceclaim`, `k8s/resourceclaimtemplate`, `k8s/resourceslice`, `k8s/inferencepool`, `k8s/inferenceobjective`, and `k8s/endpointpicker`. If an icon is not available, omit the placeholder and keep the text label. `nodes/edges` remain as a legacy fallback for simple graphs.',
  },
  {
    type: "FileEditor",
    category: "domain",
