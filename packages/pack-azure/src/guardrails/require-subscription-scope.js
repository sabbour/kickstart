 ...
 */
const SUBSCRIPTION_UUID_RE = /^\/subscriptions\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}/i;
...
export const requireSubscriptionScopeGuardrail = {
    id: 'azure/require-subscription-scope',
    appliesTo: ['*'],
    stages: ['tool'],
    async evaluate(input) {
        const toolName = input.toolName;
        if (!toolName || !AZURE_PATH_TOOLS.has(toolName))
            return { verdict: 'pass' };
        const args = input.toolArgs ?? {};
        const rawPath = args['path'] ?? args['scopePath'];
        if (!rawPath)
            return { verdict: 'pass' };
        let decoded;
        try {
        ...
        if (!SUBSCRIPTION_UUID_RE.test(decoded)) {
            return {
                verdict: 'block',
                reason: `ARM path "${decoded}" is not subscription-scoped. ` +
...
//# sourceMappingURL=require-subscription-scope.js.map