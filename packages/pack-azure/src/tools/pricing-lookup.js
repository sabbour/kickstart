...
// ── Tool ──────────────────────────────────────────────────────────────────────
export const pricingLookupTool = {
    name: 'azure.pricing_lookup',
    tool: tool({
        name: 'azure.pricing_lookup',
        description: 'Looks up Azure retail prices for a service and optional SKU using the Azure Retail Prices API. ' +
        ...
        execute: async (input) => {
            ...
            const items = (data.Items ?? []).slice(0, 20).map((item) => PricingItemSchema.parse(item));
            return {
                items,
                count: items.length,
                currencyCode: input.currencyCode ?? 'USD',
...
//# sourceMappingURL=pricing-lookup.js.map