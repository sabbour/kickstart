...
const WhatIfInputSchema = z.object({
    scopePath: z
        .string()
        .describe('ARM scope path for the deployment (resource group or subscription level). ' +
...
// ── Tool ──────────────────────────────────────────────────────────────────────
export const whatIfTool = {
    name: 'azure.what_if',
    tool: tool({
        name: 'azure.what_if',
        description: 'Runs an ARM deployment what-if analysis to preview changes before deploying. ' +
...
async function pollWhatIfResult(operationUrl, token, maxAttempts = 20) {
...
function parseWhatIfResponse(data) {
    const rawChanges = (data.properties?.changes ?? []);
    const changes = rawChanges.map((c) => WhatIfChangeSchema.parse({
