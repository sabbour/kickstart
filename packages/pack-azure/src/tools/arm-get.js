 ...
 */
export const ARM_PATH_RE = /^\/subscriptions\/[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}(?:\/resourceGroups\/[^/]+)?(?:\/providers\/[^/]+\/[^/]+\/[^/]+(?:\/[^/]+\/[^/]+)*)?$/i;
/**
 * Denylist: rejects path traversals and privileged control-plane paths.
 */
export const ARM_PATH_DENY = /(\.\.|%2e%2e|%252e|\/\/|microsoft\.authorization\/roleassignments|microsoft\.authorization\/roledefinitions)/i;
export function validateArmPath(rawPath) {
...
// ── Tool ──────────────────────────────────────────────────────────────────────
export const armGetTool = {
    name: 'azure.arm_get',
    tool: tool({
        name: 'azure.arm_get',
        description: 'Reads an Azure resource via the ARM REST API. ' +
        ...
        execute: async (input, runCtx) => {
            ...
            const token = session?.tokens?.['azure']
                ?? session?.tokens?.['azure-token'];
            if (!token) {
...
//# sourceMappingURL=arm-get.js.map