...
const EmitUiInputSchema = z.object({
    message: z
        .unknown()
        .describe('A valid A2UI v0.9 message envelope. Must have a "version" field equal to "v0.9" ' +
...
export const emitUiTool = {
    ...
    tool: tool({
        name: 'core.emit_ui',
        description: 'Validates and emits an A2UI v0.9 message. ' +
        ...
        execute: async (input, runCtx) => {
            ...
            let parsed;
            try {
                parsed = A2UIMessageSchema.parse(input.message);
            }
            catch (err) {
