...
const A2UIActionSchema = z.object({
    event: z.object({
        ...
        payload: z
            ...
            .nullable()
            .describe('Event payload with a closed key set (confirmed, id, value, action, target). ' +
...
}).describe('Interaction descriptor. Use `action: { event: { name: "..." } }` — never a bare onClick string.');
...
const A2UIComponentSchema = z.discriminatedUnion('component', [
    ...
    z.object({
        ...
        children: z.array(z.string()).describe('Ordered array of child component IDs. Use for horizontal layouts.'),
    }).strict(),
    z.object({
        id: z.string().describe('Unique component ID within this surface.'),
        component: z.literal('Column'),
        children: z.array(z.string()).describe('Ordered array of child component IDs. Use for vertical layouts.'),
    }).strict(),
    z.object({
        id: z.string().describe('Unique component ID within this surface.'),
    ...
    z.object({
        ...
        child: z.string().describe('ID of the Text (or Icon) child component that provides the button label. ' +
    ...
    z.object({
        ...
        rows: z.array(z.array(A2UIDynamicString)).describe('Table rows; each row is an array of cell values matching the columns order.'),
    }).strict(),
    z.object({
        id: z.string().describe('Unique component ID within this surface.'),
        component: z.literal('Alert'),
        message: A2UIDynamicString.describe('Alert message text.'),
        action: A2UIActionSchema.nullable().describe('Optional action on the alert (e.g. dismiss button), or null.'),
    }).strict(),
    ...
    z.object({
        ...
        action: A2UIActionSchema.nullable().describe('Action dispatched when a selection is made, or null.'),
    }).strict(),
    z.object({
        id: z.string().describe('Unique component ID within this surface.'),
        ...
        action: A2UIActionSchema.nullable().describe('Action dispatched when a selection changes, or null.'),
    }).strict(),
    z.object({
        id: z.string().describe('Unique component ID within this surface.'),
        component: z.literal('Toggle'),
        label: A2UIDynamicString.nullable().describe('Label for the toggle, or null.'),
        checked: A2UIDynamicBoolean.nullable().describe('Current toggle state bound to the data model, or null.'),
        action: A2UIActionSchema.nullable().describe('Action dispatched when the toggle changes, or null.'),
    }).strict(),
    z.object({
        id: z.string().describe('Unique component ID within this surface.'),
    ...
    z.object({
        ...
        createSurface: z.object({
            ...
            sendDataModel: z
                .boolean()
                .nullable()
                .describe('Whether the surface expects server-pushed data-model updates. ' +
...
]).describe('A valid A2UI v0.9 message envelope. Must have "version": "v0.9" and "op" set to one of: ' +
...
export const emitUiTool = {
    name: 'core.emit_ui',
    tool: tool({
        name: 'core.emit_ui',
        description: 'Validates and emits an A2UI v0.9 message (spec: https://a2ui.org/specification/v0.9-a2ui/). ' +
        ...
        execute: async (input, runCtx) => {
            ...
            try {
                ...
                parsed = A2UIMessageSchema.parse(cleaned);
            }
            catch (err) {
            ...
            const op = Object.keys(parsed)
                .find(k => k !== 'version') ?? 'unknown';
            return `emitted: ${op}`;
        },
    }),
};
//# sourceMappingURL=emit_ui.js.map