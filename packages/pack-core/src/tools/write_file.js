...
function resolveConfinedPath(workspaceRoot, relativePath) {
    ...
    while (true) {
        try {
            ...
            if (parent === checkDir)
                throw new Error(`write_file: path escapes workspace root: ${relativePath}`, { cause: err });
            checkDir = parent;
        }
    }
...
const WriteFileInputSchema = z.object({
    path: z
        .string()
        .min(1)
        .describe('Relative path to write within the session workspace. ' +
...
export const writeFileTool = {
    ...
    tool: tool({
        name: 'core.write_file',
        description: 'Writes UTF-8 text content to a file at the given path within the session workspace. ' +
