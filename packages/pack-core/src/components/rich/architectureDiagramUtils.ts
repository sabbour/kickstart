...
export interface DiagramNode {
  ...
  label: string;
  type?: string;
  /** Optional icon key from the allowed icon registry (e.g. "azure/aks"). */
  iconKey?: string | null;
}
 ...
 *      and javascript: URIs on formaction are removed.
 *      Removing foreignObject wholesale causes blank diagram labels (bug #405).
 *
...
export function sanitizeSvgMarkup(svg: string): string {
  ...
  // and javascript: src/href URIs while keeping the element itself.
