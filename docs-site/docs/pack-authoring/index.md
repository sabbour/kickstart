...
Kickstart is built on the **harness + packs** model. All domain knowledge lives in packs; the harness is domain-agnostic. Extending Kickstart means authoring a new pack (or contributing to an existing one).

This guide walks you from zero to a working pack. If you need to build or extend a full conversation agent, see the [Agent Authoring Guide](/docs/agent-authoring).
